...
if (water),
  ...
  if (nargin < 3),
    v=ncread_int(fname,vname);
  else
    v=ncread_int(fname,vname,tindex);
  end,
  [Npts,Ntime]=size(v);
  ...
  if (nargin < 3),
    f=ncread_int(fname,vname);
  else
    f=ncread_int(fname,vname,tindex);
  end,
