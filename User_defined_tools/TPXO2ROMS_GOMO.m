%*************************************************************
function TPXO2ROMS_GOMO(t0,ROMSnames,fnGrid,fnOut,ndays,DATADIR)
...
%ROMS grid info
lonR=ncread(fnGrid,'east_e');
lonR=mod(lonR,360); %This doesn't solve the problem of grids that span longitude=0;
latR=ncread(fnGrid,'north_e');
maskR=ncread(fnGrid,'fsm');
% maskV=ncread(fnGrid,'dvm');
[Lm,Mm]=size(maskR);
L=Lm-1; M=Mm-1;

%TPXO filenames (files must be on Matlab path)
TPXOfile.grid=[DATADIR,'TPXO9/grid_tpxo9.nc'];
TPXOfile.elev=[DATADIR,'TPXO9/h_tpxo9.v1.nc'];
TPXOfile.vel= [DATADIR,'TPXO9/u_tpxo9.v1.nc'];
...
varname={'tide_period'; 'tide_Ephase_w'; 'tide_Eamp_w'; ...
    'tide_Ephase_e'; 'tide_Eamp_e'; 'tide_Ephase_s'; 'tide_Eamp_s'; ...
    'tide_Ephase_n'; 'tide_Eamp_n'};   %;...
...
if ~exist(fnOut,'file')
   ...
   attvals={{'Tide angular period','seconds'};           %tide_period
      {'Tide elevation phase angle at West Edge','rad'}; %tide_Ephase
      {'Tide elevation amplitude at West Edge','meters'};    %tide_Eamp 
      {'Tide elevation phase angle at Eest Edge','rad'}; %tide_Ephase
      {'Tide elevation amplitude at Eest Edge','meters'};    %tide_Eamp
      {'Tide elevation phase angle at South Edge','rad'}; %tide_Ephase
      {'Tide elevation amplitude at South Edge','meters'};    %tide_Eamp 
      {'Tide elevation phase angle at North Edge','rad'}; %tide_Ephase
      {'Tide elevation amplitude at North Edge','meters'}};    %tide_Eamp
...
%       {'Tidal current V-component phase','degrees'}};      %tide_Vphase
   ...
   fileschema.Name='/';
   fileschema.Dimensions(1).Name='x';
   fileschema.Dimensions(1).Length=L+1;
   fileschema.Dimensions(2).Name='y';
   ...
   fileschema.Attributes(4).Value='ROMS forcing file from TPXO9';
   ...
   for i=1:nVar
      ...
      elseif i<6
         fileschema.Variables(i).Dimensions(1).Name='y';
         fileschema.Variables(i).Dimensions(1).Length=M+1;
         fileschema.Variables(i).Dimensions(2).Name='tide_period';
         fileschema.Variables(i).Dimensions(2).Length=Nharmonics;
      else
         fileschema.Variables(i).Dimensions(1).Name='x';
...
ROMSperiods = ROMSperiods*3600.d0;
ncwrite(fnOut,'tide_period',ROMSperiods)
ncwrite(fnOut,'tide_Eamp_w',squeeze(zamp(1,:,:)))
ncwrite(fnOut,'tide_Eamp_e',squeeze(zamp(end,:,:)))
ncwrite(fnOut,'tide_Eamp_s',squeeze(zamp(:,1,:)))
ncwrite(fnOut,'tide_Eamp_n',squeeze(zamp(:,end,:)))

zpha = zpha/180.d0*pi;
ncwrite(fnOut,'tide_Ephase_w',squeeze(zpha(1,:,:)))
ncwrite(fnOut,'tide_Ephase_e',squeeze(zpha(end,:,:)))
ncwrite(fnOut,'tide_Ephase_s',squeeze(zpha(:,1,:)))
ncwrite(fnOut,'tide_Ephase_n',squeeze(zpha(:,end,:)))
...
X=ncread(TPXOfile.grid,'lon_u');
Y=ncread(TPXOfile.grid,'lat_u');
...
X=ncread(TPXOfile.grid,'lon_v');
Y=ncread(TPXOfile.grid,'lat_v');
