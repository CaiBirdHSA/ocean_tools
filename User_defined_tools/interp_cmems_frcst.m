function interp_cmems_frcst(hycom_name,Roa,interp_method,...
...
%
zeta=ext_data_hycom(nc,hycom_name,lonT,latT,'zos',tin,lon,lat,1,Roa,interp_method);
%
%bottom t,s,u,v
%
...
for k=1:NZ
  ...
  u2d=ext_data_hycom(nc,hycom_name,lonU,latU,'uo',tin,lon,lat,...
                    k,Roa,interp_method);
  v2d=ext_data_hycom(nc,hycom_name,lonV,latV,'vo',tin,lon,lat,...
                    k,Roa,interp_method);
  u(k,:,:)=rho2u_2d(u2d.*cosa+v2d.*sina);
  v(k,:,:)=rho2v_2d(v2d.*cosa-u2d.*sina);
  temp(k,:,:)=ext_data_hycom(nc,hycom_name,lonT,latT,'thetao',tin,lon,lat,...
                            k,Roa,interp_method);
  salt(k,:,:)=ext_data_hycom(nc,hycom_name,lonT,latT,'so',tin,lon,lat,...
                            k,Roa,interp_method);
end
