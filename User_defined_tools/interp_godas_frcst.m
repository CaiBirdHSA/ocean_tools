function interp_godas_frcst(hycom_name,Roa,interp_method,...
...
for k=1:NZ
  ...
  u2d=ext_data_hycom(nc,hycom_name,lonU,latU,'u',tin,lon,lat,...
                    k,Roa,interp_method);
  v2d=ext_data_hycom(nc,hycom_name,lonV,latV,'v',tin,lon,lat,...
                    k,Roa,interp_method);
  u(k,:,:)=rho2u_2d(u2d.*cosa+v2d.*sina);
  v(k,:,:)=rho2v_2d(v2d.*cosa-u2d.*sina);
  temp(k,:,:)=ext_data_hycom(nc,hycom_name,lonT,latT,'temp',tin,lon,lat,...
                            k,Roa,interp_method);
  salt(k,:,:)=ext_data_hycom(nc,hycom_name,lonT,latT,'salt',tin,lon,lat,...
                            k,Roa,interp_method);
end
