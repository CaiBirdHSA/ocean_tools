function interp_OGCM_frcst_bohai(OGCM_prefix,OGCM_name,Roa,interp_method,...
...
%
nc=netcdf([OGCM_prefix,OGCM_name]);    %nc1:el
%
...
%
zeta=ext_data_OGCM(nc,lonT,latT,'surf_el',tin,lon,lat,1,Roa,interp_method);
u2d=ext_data_OGCM(nc,lonU,latU,'u_barotropic_velocity',tin,lon,lat,1,Roa,interp_method);
v2d=ext_data_OGCM(nc,lonV,latV,'v_barotropic_velocity',tin,lon,lat,1,Roa,interp_method);
...
for k=1:NZ
  ...
  temp(k,:,:)=ext_data_OGCM(nc6,lonT,latT,'water_temp',tin,lon,lat,...
                            k,Roa,interp_method);
...
% Close the OGCM file
%
close(nc);close(nc2);close(nc3);close(nc4)
